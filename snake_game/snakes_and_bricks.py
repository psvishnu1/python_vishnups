'''
Add the bricks functionality to the snake game.

Lay bricks (red squares) along the 4 boundaries
Implement a logic by which if the head of the snake hits a brick, it dies and the game ends.
'''
    ...
    def __init__(self, numRows:int, numCols:int):
        ...
        for i in [0,self.numRows-1]:
            for j in range(self.numCols):
                self.squareOn(i,j,'red')
 
        for i in [0,self.numCols-1]:
            for j in range(self.numRows):
                self.squareOn(j,i,'red')
...
class Snake:
    ...
    def funeral(self):
        self.GameBoard.msg = tk.Label(self.GameBoard.gameWindow,text='Game Over',font=('Times',52))  
        self.GameBoard.msg.pack()
    ...
    def move(self):
        ...
        if(self.headRow == 0 or self.headCol == 0):
            self.isAlive = False
            self.funeral()
        elif(self.headRow == self.GameBoard.numRows-1 or self.headCol == self.GameBoard.numCols-1):
            self.isAlive = False
            self.funeral()
